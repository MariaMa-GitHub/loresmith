...
def upgrade() -> None:
    # Safe to drop existing rows: the cache is opportunistic and nothing
    # persists user content there.
    op.execute("DELETE FROM semantic_cache")
    op.add_column(
        "semantic_cache",
        sa.Column("corpus_revision", sa.String(length=64), nullable=False, server_default=""),
    )
    op.add_column(
        "semantic_cache",
        sa.Column("max_spoiler_tier", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "semantic_cache",
        sa.Column("embedding_backend", sa.String(length=32), nullable=False, server_default=""),
    )
    op.add_column(
        "semantic_cache",
        sa.Column("embedding_model", sa.String(length=256), nullable=False, server_default=""),
    )
    # Drop server defaults so future inserts must set scope columns explicitly.
    ...
    op.create_index(
        ...
        "semantic_cache",
        ["game_slug", "corpus_revision", "max_spoiler_tier", "embedding_backend", "embedding_model"],
    )
