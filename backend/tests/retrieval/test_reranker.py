...
async def test_cross_encoder_reranker_reorders_by_score(monkeypatch):
    ...
    # Passage 3 should win even though it's RRF-last.
    hits = [_hit(1, "alpha"), _hit(2, "beta"), _hit(3, "gamma")]

    def fake_score(pairs):
        # Higher is better. Order: gamma > alpha > beta.
        return {"alpha": 0.2, "beta": 0.1, "gamma": 0.9}
    async def fake_async(pairs):
        return [fake_score(pairs)[p[1]] for p in pairs]
